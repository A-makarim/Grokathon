...
import { z } from "zod";
import { xTools, jobRegistryTools } from "@xbounty/common";

import { CONFIG } from "../config/index.js";
import { getJobSystemPrompt, getJobPrompt } from "../prompts/index.js";
import {
...
import { generateJobId, log } from "../utils/index.js";
...
export class CreationAgent {
  ...
  async runGenerationCycle(): Promise<Job[]> {
    log.info("Starting generation cycle...");

    const systemPrompt = getJobSystemPrompt(this.config);
    const jobPrompt = getJobPrompt(this.config);

    const schema = z.object({
      jobs: z.array(GeneratedJobSchema),
    });
    ...
    // Build tools object
    const tools = {
      // REGISTRY TOOLS - Check for existing jobs before generating
      ...jobRegistryTools,
    ...
    log.info(`Registered ${Object.keys(tools).length} tools (${Object.keys(jobRegistryTools).length} registry + ${Object.keys(tools).length - Object.keys(jobRegistryTools).length} X API) + Live Search`);
