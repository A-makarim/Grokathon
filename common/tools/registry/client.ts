...
export interface RegistryStats {
  // Legacy market stats (for backward compatibility)
  totalMarkets?: number;
  openMarkets?: number;
...
// =============================================================================
// JOB MARKETPLACE CLIENT FUNCTIONS
// =============================================================================
...
export async function listJobs(options: {
  ...
  if (options.complexity && options.complexity.length > 0) {
    params.set("complexity", options.complexity.join(","));
  }
  ...
  const query = params.toString();
  const url = query ? `/jobs/open?${query}` : "/jobs/open";

  const result = await registryFetch<{ jobs: JobSummary[]; total: number }>(url);

  if (result.error) {
    return { error: result.error };
  }

  return result.data || { jobs: [], total: 0 };
}

/**
 * Search jobs by keyword (client-side filtering)
 */
export async function searchJobs(
  ...
  const searchLower = searchTerm.toLowerCase();
  const filtered = result.jobs.filter(
    (job) =>
...
export async function getJob(jobId: string): Promise<{
  ...
  const result = await registryFetch<JobSummary>(`/jobs/${jobId}`);
